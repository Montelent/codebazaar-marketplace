...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [form, setForm] = useState({
    title: detail.title, slug: detail.slug, description: detail.descriptionHtml,
    regularPrice: String(detail.regularPrice), extendedPrice: String(detail.extendedPrice),
    salePriceRegular: detail.salePriceRegular != null ? String(detail.salePriceRegular) : "",
    categorySlug: detail.category.slug, thumbnailUrl: detail.thumbnailUrl,
    demoUrl: detail.demoUrl || "", mainFileUrl: "", isFree: Number(detail.regularPrice) === 0,
  });
  const [screenshots, setScreenshots] = useState<string[]>(detail.galleryUrls?.length ? detail.galleryUrls : []);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const slug = prod.product?.category?.slug || detail.category.slug || "javascript";
      ...
      if (prod.product) {
        ...
        setForm({
          title: p.title || detail.title, slug: p.slug || detail.slug,
          ...
          categorySlug: slug, thumbnailUrl: p.thumbnailUrl || detail.thumbnailUrl,
          demoUrl: p.demoUrl || "", mainFileUrl: (p as { mainFileUrl?: string }).mainFileUrl || "",
          isFree: free,
        });
        ...
        for (const a of p.attributes || []) {
          ...
          if (presets[a.label]) {
            selected[a.label] = String(a.value).split(",").map((x: string) => x.trim()).filter(Boolean);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      const body = {
        id, title: form.title, slug: form.slug, description: form.description,
        ...
        categorySlug: form.categorySlug, thumbnailUrl: form.thumbnailUrl, demoUrl: form.demoUrl,
        mainFileUrl: form.mainFileUrl || undefined, galleryUrls: screenshots,
        features: features.filter(Boolean), tags: selectedTags, attributes: buildAttributes(),
      };
      ...
      setMsg("Saved.");
      setUpdatedLabel(new Date().toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" }));
    } catch (err) {
  ...
  if (booting) return <div className="p-8 text-sm text-slate-500">Loading product…</div>;
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <form onSubmit={onSubmit} className="space-y-6">
        <section className="space-y-4 rounded-xl border bg-white p-6 shadow-sm">
          ...
          <Field label="Description"><CKEditorField value={form.description} onChange={(v) => set("description", v)} minHeight={200} /></Field>
          <label className="flex items-center gap-2 text-sm font-medium">
            <input type="checkbox" checked={form.isFree} onChange={(e) => set("isFree", e.target.checked)} className="accent-emerald-600" /> Free product
          </label>
          {!form.isFree && (
        ...
        <section className="space-y-4 rounded-xl border bg-white p-6 shadow-sm">
          ...
          <Field label="Live demo URL"><Input type="url" value={form.demoUrl} onChange={(e) => set("demoUrl", e.target.value)} /></Field>
          <Field label="Main download file URL">
            ...
            <p className="mt-1 text-xs text-slate-500">Host the ZIP on storage and paste the URL (best for large files).</p>
          </Field>
        </section>
        ...
        <section className="space-y-3 rounded-xl border bg-white p-6 shadow-sm">
          ...
          <select className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm" value={form.categorySlug} onChange={(e) => onCategoryChange(e.target.value)}>
            {categories.length === 0 && <option value={form.categorySlug}>{form.categorySlug}</option>}
            {categories.map((c) => <option key={c.slug} value={c.slug}>{c.name}</option>)}
          </select>
          <p className="text-xs text-slate-500">Attributes switch to this category's set.</p>
        </section>
        ...
        <section className="space-y-4 rounded-xl border bg-white p-6 shadow-sm">
          ...
          <div className="border-t border-slate-100 pt-4">
            ...
            <Button type="button" variant="outline" size="sm" onClick={() => setCustomAttrs((a) => [...a, { label: "", value: "" }])}><Plus className="h-4 w-4" /> Add custom</Button>
          </div>
        </section>
        ...
        <section className="space-y-2 rounded-xl border border-slate-200 bg-slate-50 p-4">
          <p className="text-sm text-slate-600"><span className="font-medium">Created:</span> {createdLabel}</p>
          <p className="text-sm text-slate-600"><span className="font-medium">Last update:</span> {updatedLabel}</p>
        </section>
        ...
        <div className="flex gap-3">
          ...
          <Link href={`/item/${form.slug}/${id}`} target="_blank"><Button type="button" variant="outline">View on site</Button></Link>
        </div>
      </form>
    </div>
  );
