...
export default function ItemDetailPage({
    ...
    fetch(`/api/products/${id}?slug=${encodeURIComponent(slug || "")}`)
      ...
      .then((data) => {
        ...
        setProduct({
          ...base,
          ...p,
          features: p.features?.length ? p.features : base.features,
          requirements: p.requirements?.length ? p.requirements : base.requirements,
          changelogs: p.changelogs?.length ? p.changelogs : base.changelogs,
          licenseFeatures: p.licenseFeatures?.length
            ? p.licenseFeatures
            : base.licenseFeatures,
          attributes: p.attributes?.length ? p.attributes : base.attributes,
          tags: p.tags?.length ? p.tags : base.tags,
          galleryUrls: p.galleryUrls?.length ? p.galleryUrls : base.galleryUrls,
  ...
  const authorItems = MOCK_ITEMS.filter(
    (i) =>
      i.author.username === product.author.username &&
      i.id !== product.id &&
      i.slug !== product.slug
  ).slice(0, 4);
        ...
        <div className="mx-auto flex max-w-7xl items-start justify-between gap-4 px-4 py-4">
          <div className="min-w-0 flex-1">
            <nav className="mb-1 flex flex-wrap items-center gap-1 text-xs text-slate-500">
              ...
              <Link
                href={`/category/${product.category?.slug || "code"}`}
                className="hover:text-emerald-600"
              >
            ...
            <div className="mt-1 flex flex-wrap items-center gap-3 text-sm text-slate-600">
              <span>
                By{" "}
                <Link
                  href={`/author/${product.author?.username || "codebazaar"}`}
              ...
              {!loaded && <span className="text-xs text-slate-400">Updating…</span>}
            </div>
          </div>
          <div className="hidden shrink-0 items-center gap-2 sm:flex">
            ...
            <Link
              href="/cart"
              className="relative flex items-center gap-1 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-semibold text-slate-800 hover:bg-slate-100"
            >
      ...
      <div className="mx-auto grid max-w-7xl gap-8 px-4 py-6 lg:grid-cols-12">
        <div className="min-w-0 lg:col-span-8">
          <div className="mb-4 flex items-center gap-2 border-b border-slate-200">
            <button
              type="button"
              className="border-b-2 border-emerald-600 px-1 pb-2 text-sm font-semibold text-slate-900"
            >
        ...
        <aside className="lg:col-span-4">
          <div className="sticky top-20 space-y-4 overflow-visible">
            <div className="relative z-20 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                ...
                {!isFree && (
                  <li className="flex items-start gap-2 text-sm text-slate-700">
                    ...
                    <label htmlFor="extend-support" className="cursor-pointer">
                      ...
                      <span className="font-semibold text-slate-800">$14.50</span>
                    </label>
                  </li>
                )}
              </ul>
              ...
              <Button
                className={cn("mt-4 h-12 w-full text-base font-semibold", added && "bg-emerald-800")}
                onClick={handleAdd}
              >
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <Link
                href={`/author/${product.author?.username || "codebazaar"}`}
                className="mt-3 flex w-full items-center justify-center rounded-md border border-slate-300 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50"
              >
                View Portfolio
              </Link>
            </div>
