...
function emptyProduct(id: string, slug: string): ProductDetail {
...
export default function ItemDetailPage({
  ...
  const card = MOCK_ITEMS.find((i) => i.id === id || i.slug === slug);
  const [product, setProduct] = useState<ProductDetail>(() =>
    card ? PRODUCT_DETAILS[card.id] ?? detailFromCard(card) : emptyProduct(id, slug)
  );
  ...
  useEffect(() => {
    let cancelled = false;
    setLoaded(false);
    setProduct(card ? PRODUCT_DETAILS[card.id] ?? detailFromCard(card) : emptyProduct(id, slug));
    fetch(`/api/products/${id}?slug=${encodeURIComponent(slug || "")}`)
      ...
      .then((data) => {
        ...
        setProduct({
          ...
          thumbnailUrl:
            p.thumbnailUrl ||
            "https://picsum.photos/seed/" + (p.slug || slug) + "/640/400",
          ...
          changelogs: Array.isArray(p.changelogs) ? p.changelogs : [],
          licenseFeatures: Array.isArray(p.licenseFeatures) ? p.licenseFeatures : [],
          attributes: Array.isArray(p.attributes) ? p.attributes : [],
          tags: Array.isArray(p.tags) ? p.tags : [],
          galleryUrls: Array.isArray(p.galleryUrls) ? p.galleryUrls : [],
          lastUpdate: p.lastUpdate || "—",
          createdAt: p.createdAt || "—",
          category: p.category || { name: "Code", slug: "code" },
          author: p.author || {
            username: "codebazaar",
            displayName: "CodeBazaar",
          },
        ...
        <div className="mx-auto flex max-w-7xl items-start justify-between gap-4 px-4 py-4">
          <div className="min-w-0 flex-1">
            ...
            <h1 className="text-xl font-semibold text-slate-900 sm:text-2xl">
              {product.title || (loaded ? "Product" : "Loading…")}
            </h1>
